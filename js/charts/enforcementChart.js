...
// Diverging stacked bar chartPolice (left) vs Camera (right)
...
import { renderStory, ensureStoryPanel, getEnforcementStory, getDataNotes } from './storyTelling.js';

const CAMERA_START_YEAR = 2020;

// Enforcement-specific data notes drawn from the data dictionary
const ENFORCEMENT_DATA_NOTES = [
'📷 Camera fines are collected by different agencies per stateNSW Revenue (NSW), Dept. of Transport (QLD), Access Canberra (ACT seatbelt). Police figures come from each state\'s Police Force.',
'⚠️ Victoria cannot provide fines, arrests or charges for automated detectionsits database only stores detection data. VIC camera figures here reflect available fine data only.',
'🛑 WA camera detections result in <strong>charges rather than fines</strong>the camera "fines" bar for WA will read zero; the charges column tells the full story.',
'📅 2024 Victorian data volumes were low due to <strong>Protected Industrial Action (PIA)</strong>treat 2024 VIC totals with caution.',
'🔢 Data is subject to revision. Some states use proxy counts (sum of fines + charges) where raw count data was unavailable.',
];

export function createEnforcementChart(containerId = 'enforcement-chart') {
ensureStoryPanel('enforcement', 'enforcement-story-panel', 'enforcement-chart');
...
// ── Story + data notes ────────────────────────────────────────────────
// Show the story for the most recent year in the chart (2024)
renderStory(
'enforcement-story-panel',
getEnforcementStory(2024),
ENFORCEMENT_DATA_NOTES
);
